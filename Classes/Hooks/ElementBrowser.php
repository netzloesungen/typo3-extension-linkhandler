...
class ElementBrowser implements \TYPO3\CMS\Core\ElementBrowser\ElementBrowserHookInterface {
	...
	public function modifyMenuDefinition($menuDef) {
		...
		foreach ($tabs as $key => $tabConfig) {
			...
			$addPassOnParams = GeneralUtility::implodeArrayForUrl(
				'',
				GeneralUtility::explodeUrl2Array($addPassOnParams),
				'',
				TRUE
			);
			$htmlAreaParams = explode(':', $this->pObj->bparams[1]);

			$menuDef[$key]['addParams'] = 'onclick="jumpToUrl(\'?act=' . $key . '&editorNo=' . $htmlAreaParams[0] . '&contentTypo3Language=' . $htmlAreaParams[1] . '&contentTypo3Charset=' . $htmlAreaParams[2] . $addPassOnParams . '\');return false;"';
		}
	...
	public function getTab($act) {
		...
		$tabHandler = new $tabHandlerClass(
			$this->pObj,
		...
		if (!$tabHandler instanceof TabHandlerInterface) {
			throw new \UnexpectedValueException(
				'$tabHandler must implement interface AOE\\Linkhandler\\TabHandlerInterface',
				1412524271
			);
		}
	...
	protected function getAllRegisteredTabHandlerClassnames() {
		...
		if (is_array($GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['linkhandler/class.tx_linkhandler_browselinkshooks.php'])) {
			foreach (
				$GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['linkhandler/class.tx_linkhandler_browselinkshooks.php'] as
				$tabHandler
			) {
				list($file, $class) = GeneralUtility::revExplode(':', $tabHandler, 2);
				$default[] = $class;
			}
		}
	...
	private function _checkConfigAndGetDefault() {
		...
		if ($this->pObj->mode == 'rte') {
			...
			$this->pObj->thisConfig = BackendUtility::RTEsetup(
				$RTEsetup['properties'],
				$RTEtsConfigParts[0],
				$RTEtsConfigParts[2],
				$RTEtsConfigParts[4]
			...
			$modTSconfig = $GLOBALS["BE_USER"]->getTSConfig("mod.tx_linkhandler", BackendUtility::getPagesTSconfig($pid));
